...
io.on('connection', function (socket) {
  ...
  socket.on('self', function (data) {
    ...
    socket.emit('message', data +" From Me ("+i+")")
    i++;
  });
  ...
  socket.on('all', function (data) {
    ...
    // in all rooms
    io.sockets.emit('message', data +' To everyone! (' + i + ')')
    i++;
  });
  ...
  socket.on('broadcast', function (data) {
    ...
    socket.broadcast.emit('message', data+ ' To everyone but me! (' + i + ')')
    i++;
  });
  ...
  socket.on('message', function (data) {
    ...
    io.sockets.in(socket.room).emit( 'message', data+' To everyone in ' + socket.room + '! (' + i + ')' );
    i++;
  });
  ...
  socket.on('broadcastRoom', function (data) {
    ...
    socket.broadcast.to(socket.room).emit( 'message', data+' To everyone in ' + socket.room + ' but origin! (' + i + ')' );
    i++;
  });
})
